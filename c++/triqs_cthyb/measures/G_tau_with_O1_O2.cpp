...
namespace triqs_cthyb {
  ...
  void measure_G_tau_with_O1_O2::accumulate(mc_weight_t s) {
    ...
    for (auto block_idx : range(G_tau_with_O1_O2.size())) {
      ...
      // Computing: ⟨O1 c_β(τ_y) O2 c†_α(τ_x)⟩ - ⟨c_β(τ_y) O1 O2 c†_α(τ_x)⟩ - ⟨O1 c_β(τ_y) c†_α(τ_x) O2⟩ + ⟨c_β(τ_y) O1 c†_α(τ_x) O2⟩
      foreach (data.dets[block_idx], [this, s, block_idx, baseline_trace](op_t const &x, op_t const &y, det_scalar_t M) {
        ...
        // Case 1: O1 inserted AFTER c (at τ_y+ε), O2 inserted AFTER c† (at τ_x+ε)
        // Measures: ⟨O1 c_β(τ_y) O2 c†_α(τ_x)⟩
        ...
        try {
          ...
          data.imp_trace.try_insert(tau_O1_after, op1_d);
          data.imp_trace.try_insert(tau_O2_after, op2_d);
        ...
        try {
          // Insert both operators into impurity trace
          data.imp_trace.try_insert(tau_O1_before, op1_d);
          data.imp_trace.try_insert(tau_O2_before, op2_d);
        ...
        try {
          // Insert both operators into impurity trace
          data.imp_trace.try_insert(tau_O1_case3, op1_d);
          data.imp_trace.try_insert(tau_O2_case3, op2_d);
        ...
        try {
          // Insert both operators into impurity trace
          data.imp_trace.try_insert(tau_O1_case4, op1_d);
          data.imp_trace.try_insert(tau_O2_case4, op2_d);
        ...
        try {
          // Insert only O1
          data.imp_trace.try_insert(tau_O1_only_after, op1_d);
        ...
        try {
          // Insert only O1
          data.imp_trace.try_insert(tau_O1_only_before, op1_d);
        ...
        try {
          // Insert only O2
          data.imp_trace.try_insert(tau_O2_only_after, op2_d);
        ...
        try {
          // Insert only O2
          data.imp_trace.try_insert(tau_O2_only_before, op2_d);
